...
"""
kp2gaussian 函数
这个函数接受三个参数：kp（关键点张量），spatial_size（空间尺寸，通常是一个元组，包含图像的高度和宽度），以及 kp_variance（关键点的方差）。函数的目的是将关键点转换为一个高斯分布，这在计算机视觉任务中，如目标检测或姿态估计，用于表示关键点周围的不确定性。

使用 make_coordinate_grid 函数创建一个坐标网格，这个网格包含了图像中所有可能的坐标点。
根据关键点张量的形状，调整坐标网格的形状，使其与关键点张量兼容。
将坐标网格重复关键点张量中的领先维度次数，以便在每个关键点周围生成高斯分布。
将关键点坐标从像素坐标转换为标准化的 [-1, 1] 范围的坐标。
计算每个网格点与关键点之间的差异（mean_sub），并使用这个差异来创建高斯分布。高斯分布的中心是关键点坐标，方差由 kp_variance 参数控制。
返回生成的高斯分布张量。
"""
def kp2gaussian(kp, spatial_size, kp_variance):
    ...
    #先调用下面的函数生成网格坐标（可以理解为归一化的网格坐标）
    coordinate_grid = make_coordinate_grid(spatial_size, kp.type()).to(kp.device)
    #这个计算的是关键点表示的维度与网格坐标维度的差值
    #为了将其与关键点张量的形状对齐，我们需要在坐标网格的前面添加足够的维度，使其形状与关键点张量的领先维度相匹配。
    # number_of_leading_dimensions 就是用来确定需要添加多少个维度的
...
def make_coordinate_grid(spatial_size, type):
    ...
    #将长和宽线性映射到[-1,1]之间
    x = (2 * (x / (w - 1)) - 1)  #[-1.0000, -0.7143, -0.4286, -0.1429,  0.1429,  0.4286,  0.7143,  1.0000]
    y = (2 * (y / (h - 1)) - 1)  #[-1.0000, -0.6000, -0.2000,  0.2000,  0.6000,  1.0000]

    yy = y.view(-1, 1).repeat(1, w)
    xx = x.view(1, -1).repeat(h, 1)

    #结果是映射完的长和宽两两组合，以上面为例组合成8*6=48对，形状是[w,h,2]
...
class Decoder(nn.Module):
    ...
    def __init__(self, block_expansion, in_features, num_blocks=3, max_features=256):
        ...
        #print("block_expansion:",block_expansion,"in_features:",in_features)
