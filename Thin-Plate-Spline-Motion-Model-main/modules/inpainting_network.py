...
class InpaintingNetwork(nn.Module):
    ...
    def occlude_input(self, inp, occlusion_map):
        ...
        #对图像做遮挡的核心操作其实就是图像*mask矩阵，相关的实践可以看 图像遮挡掩膜的示例.py 文件
    ...
    def forward(self, source_image, dense_motion):
        """
        先明确一下各个列表的作用：
        encoder_map 用来存放下采样的特征图
        warped_encoder_maps 用来存放源图像按光流变换后与掩码作用后的结果

        """
        #对输入图像做卷积并归一化
        out = self.first(source_image)

        #逐步对图像下采样，encode_map存储下采样路径的特征图
        encoder_map = [out]
        ...
        # 这个整个过程对应figure3的示意图，但这个和示意图不太一样，这个代码是先把残差和上采样做了
        # 就是画的应该把Contect上面的部分移到Upsampling下面
        for i in range(self.num_down_blocks):
            ...
            #这句话对应示意图中的”pix-wise multiplication“
        ...
        #输出由本来的out与encoder最后一层编码器的结果线性组合而成
