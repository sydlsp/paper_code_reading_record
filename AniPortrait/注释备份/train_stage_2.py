...
def main(cfg):
    ...
    #sched_kwargs来源于stage2.yaml的noise_scheduler_kwargs
    #感觉这里是DDIM去噪器
    val_noise_scheduler = DDIMScheduler(**sched_kwargs)
    sched_kwargs.update({"beta_schedule": "scaled_linear"})
    train_noise_scheduler = DDIMScheduler(**sched_kwargs)

    #初始化图像编码器，变分自编码器，reference_net，denoising_unet，pose_guider
    ...
    #TODO:明确一下reference_control_writer以及reference_control_reader的作用
    ...
    # 检查配置中是否启用了XFormers的内存高效注意力机制:
    # 如果启用了，并且检测到系统中已安装了XFormers库，那么会分别启用reference_unet和denoising_unet的内存高效注意力机制
    ...
    #同样的，对reference_unet和denoising_unet启动梯度检查点
    ...
    # 计算每个训练周期的更新步数：batch数/在执行一次参数更新前要积累批次的数目
    # 梯度累积策略可以模拟更大批量的训练而不需要增加实际内存的使用
    num_update_steps_per_epoch = math.ceil(
        len(train_dataloader) / cfg.solver.gradient_accumulation_steps
    )
    # Afterwards we recalculate our number of training epochs
    # 计算总的训练周期轮数(算有多少个epoch)：
    # 根据总的最大训练步数/每一轮训练的步数
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                with torch.no_grad():
                    ...
                    # 从[batch,frames,channels,height,width]
                    # 转化为 [batch_size*frames,channels,height,width]
                    ...
                    # 关于这个4是不是固定的还要再考证一下
                    # 下面再把形状转变为[batch_size,channels,frames,height,width]，这里channels=4
                    latents = rearrange(
